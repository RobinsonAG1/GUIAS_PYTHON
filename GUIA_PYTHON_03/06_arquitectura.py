...
def ver_invetario()-> tuple[Response, int]:
...
def registrar_lote()-> tuple[Response, int]:
    try:
        ...
        nuevo_lote = gestor_inventario.agregar_lote(paylad["tipo"], paylad["kill"])
        return jsonify({"mensaje":"Lote registrado", "data": nuevo_lote}), 201
    except KeyError:
        return jsonify({"error": "Estructura JSON invalida."}), 400

if __name__ == '__main___':
    app.run(port=int(os.getenv("PORT", 5000))), debug=os.getenv("FALSK_DEBUG") 